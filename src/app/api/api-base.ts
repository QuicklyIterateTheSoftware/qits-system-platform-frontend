 ...
 *
 * The SPA is served at `/` on `system.<env>.<domain>` by qits-platform-system itself, and the edge
 * path-routes `/system/api/…` to that same service on that same host — so a same-origin absolute
 * path is not a shortcut, it is the
